...
structure tactic.rewrite_search.collect_config :=
(suggest         : list name := [])
(inflate_rws     : bool := ff)
(help_me         : bool := ff)
...
iconfig_add_struct rewrite_search tactic.rewrite_search.config
iconfig_add_struct rewrite_search tactic.rewrite_search.collect_config
...
meta def instantiate_modules (st mt tr : pexpr) : tactic module_stack := do
  ulift.up e ← tactic.up $ (do
    let ns := [`strategy, `metric, `tracer].map $ λ n, `tactic.rewrite_search ++ n,
    ...
    st ← munify_app `tactic.rewrite_search.strategy [α, β, γ, δ] st,
    mt ← munify_app `tactic.rewrite_search.metric   [α, β, γ, δ] mt,
    tr ← munify_app `tactic.rewrite_search.tracer   [α, β, γ, δ] tr,

    [α, β, γ, δ, st, mt, tr] ← [α, β, γ, δ, st, mt, tr].mmap instantiate_mvars,

    s ← mk_app `tactic.rewrite_search.sig.mk [α, β, γ, δ],
    [s, st, mt, tr] ← pure $ [s, st, mt, tr].map pexpr.of_expr,
    to_expr $ ``(tactic.rewrite_search.module_stack.mk) s st mt tr
  ),
