...
meta def bfs_startup (cfg : iconfig.result) (g : search_state bfs_state β γ δ) (m : metric bfs_state β γ δ) (l r : vertex) : tactic (search_state bfs_state β γ δ) := do
  cfg ← cfg.struct `tactic.rewrite_search.strategy.bfs.bfs_config bfs_config,
...
meta def bfs (conf : iconfig.result := iconfig.empty_result) : tactic expr :=
  generic_arg `tactic.rewrite_search.strategy.bfs_const `(conf)

section

iconfig_mk bfs
iconfig_add_struct bfs tactic.rewrite_search.strategy.bfs.bfs_config
