...
meta def pexplore_startup_conf (res : iconfig.result) (g : search_state pexplore_state β γ δ) (m : metric pexplore_state β γ δ) (l r : vertex) : tactic (search_state pexplore_state β γ δ) := do
  conf ← res.struct `tactic.rewrite_search.strategy.pexplore.pexplore_config pexplore_config,
...
meta def pexplore (conf : iconfig.result := iconfig.empty_result) : tactic expr :=
  generic_arg `tactic.rewrite_search.strategy.pexplore_cnst `(conf)

section

iconfig_mk pexplore
iconfig_add_struct pexplore tactic.rewrite_search.strategy.pexplore.pexplore_config
